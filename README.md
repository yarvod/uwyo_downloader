...
Графическое приложение для быстрой загрузки радиозондовых профилей с `weather.uwyo.edu`. Сохраняет станции и выгруженные профили в локальную SQLite-базу (по умолчанию рядом с исполняемым файлом, при недоступности пути — в `~/.local/share/uwyo-sounding`; можно задать `UWYO_APP_DATA`), позволяет искать станции по ID/имени, фильтровать загруженные данные и скачивать пакетом с прогресс-баром и возможностью отмены. Разработано yarvod.
